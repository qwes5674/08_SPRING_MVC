...
public class FirstController {
    ...
    * HttpServletRequest, HttpsServletResponse/ ServletRequest, ServletResponse 가능하다.
    * */
    ...
    * from의 name 속성값과 매개변수 이름이 다른 경우 @RequestParam("이름") 설정하면 된다.
    *
    * 생략이 가능하다,. 하지만 가독성을 위해서 써주는게 좋다.
    *
    * required 속성 : 값이 들어오지 않아도 400 에러를 발생시키지 않는다.
    ...
    *
    * @ModelAttribute의 경우 커맨드객체를 생성하여 매개변수로 전달 해 준 뒤 인스턴스를 model에 담는다
    * 경우에 따라 폼에서 입력한 값을 다음 화면으로 바로 전달해야 할 경우가 발생하는데, 이때 유용하게 사용된다.
    * @ModelAttribute(" 모델에 담을 key값 ")을 지정 할 수 있으며
    * 지정하지 않으면 타입의 앞글자를 소문자로 한 네이밍 규칙을 따른다.
    * -> menuDTO
    *
    * 해당 어노테이션은 생략이 가능하지만 명시적으로 작성하는 것이 좋다.
    * 이때 주의할 점은 DTO와 입력받을 폼이 잘 맞춰줘야 한다.
    * */
    ...
    * */
    ...
    /*
    * @SessionAttribute를 이용하여 session에 값 담기
    * 클래스 레벨에 @SessionAttributes 어노테이션을 이용하여 세션에 담을 key 값을 설정
    ...
    * */
    ...
    public void body(){}
        ...
        /*
        * @RequestBody
        * - 해당 어노테이션은 http 본문 자체를 읽는 부분을 모델로 반환시켜주는
        * */
